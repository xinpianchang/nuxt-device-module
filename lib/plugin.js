...
// @add by chengyu
function isVmovierApp (a) {
  return /VmovierApp/i.test(a)
}

// @add by chengyu
function isXpcApp (a) {
  return /NewStudios/i.test(a)
}
...
function extractDevices (ctx, userAgent = DEFAULT_USER_AGENT) {
  ...
  // @add by chengyu
  const weibo = isWeibo(userAgent)
  const weixin = isWeixin(userAgent)

  // custom App for xinpianchang
  const xpcApp = isXpcApp(userAgent)
  const vmovierApp = isVmovierApp(userAgent)
...
export default async function (ctx, inject) {
  const makeFlags = () => {
    ...
    return {
      ...
      isWeibo: weibo,
      isWeixin: weixin,
      isAndroidWeixin: weixin && android,
      isIosWeixin: weixin && ios,
      isXpcApp: xpcApp,
      isAndroidXpcApp: xpcApp && android,
      isIosXpcApp: xpcApp && ios,
      isVmovierApp, vmovierApp,
      isWindows: windows,
      isMacOS: macOS,
